...
| **Local** | `.ywcoder/settings.local.json`（项目目录） | 当前用户，当前项目 | No (gitignored) |
| **Project (Shared)** | `.ywcoder/settings.json`（项目目录） | 所有协作者 | Yes (git) |
| **User (Global)** | `~/.ywcoder/settings.json` 或 `~/.ywcoder/settings.{profile}.json` | 当前用户，所有项目 | No |
| **Default** | 内置默认值 | 所有人 | — |

**系统目录路径：**
- macOS: `/Library/Application Support/YwCoderCode/`
- Linux: `/etc/ywcoder-code/`
- Windows: `C:\Program Files\YwCoderCode\`
...
管道 C: MCP Config (~/.ywcoder.json + .mcp.json)
  ├── 写入: CLI 命令 (ywcoder mcp add/remove) 或直接操作文件
  ├── 读取: 解析 ~/.ywcoder.json 和 .mcp.json
...
```
~/.ywcoder/kimi.json = 当前活跃 Profile 的 settings.json 的副本
...
|-----|------|------|-------|---------|
| `model` | string | 覆盖默认模型（alias 或完整 model ID） | `"opus"`, `"ywcoder-sonnet-4-5-20250929"` | Models |
...
| `language` | string | YwCoder 响应语言偏好 | `"japanese"` | General |
...
| `attribution` | `{ commit?: string, pr?: string }` | Git commit/PR 归因自定义 | `{"commit": "Generated with YwCoder", "pr": ""}` | General |
| `includeCoAuthoredBy` | boolean | **Deprecated**: 使用 `attribution` 代替 | `false` | — |
| `cleanupPeriodDays` | number | 非活跃会话删除天数（0=立即） | `20` | General |
| `forceLoginMethod` | `"ywcoderai" \| "console"` | 限制登录类型 | `"ywcoderai"` | General |
...
> **注意：** MCP 服务器的实际配置存储在 `~/.ywcoder.json` 和 `.mcp.json` 中（管道 C），
...
```jsonc
{
  ...
  "extraKnownMarketplaces": {
    "acme-tools": {
      "source": { "source": "github", "repo": "acme-corp/ywcoder-plugins" }
    }
  },
  "skippedMarketplaces": [],
  "skippedPlugins": [],
...
| `YWCODER_CODE_SUBAGENT_MODEL` | 子代理使用的模型 |
| `YWCODER_CODE_EFFORT_LEVEL` | Opus 4.6 effort level: `low`, `medium`, `high` |
| `MAX_THINKING_TOKENS` | Extended thinking token 上限（0=禁用） |
| `YWCODER_CODE_MAX_OUTPUT_TOKENS` | 最大输出 token（默认 32000，最大 64000） |
| `YWCODER_CODE_FILE_READ_MAX_OUTPUT_TOKENS` | 文件读取 token 上限 |
...
| `NO_PROXY` | 绕过代理的域名/IP |
| `YWCODER_CODE_PROXY_RESOLVES_HOSTS` | 允许代理 DNS 解析 |
| `YWCODER_CODE_CLIENT_CERT` | mTLS 客户端证书文件 |
| `YWCODER_CODE_CLIENT_KEY` | mTLS 客户端私钥文件 |
| `YWCODER_CODE_CLIENT_KEY_PASSPHRASE` | mTLS 私钥密码 |

### 3.4 云供应商（Network Tab / Advanced）

| 环境变量 | 描述 |
|---------|------|
| `YWCODER_CODE_USE_BEDROCK` | 使用 AWS Bedrock |
| `YWCODER_CODE_SKIP_BEDROCK_AUTH` | 跳过 Bedrock AWS 认证 |
| `AWS_BEARER_TOKEN_BEDROCK` | Bedrock API key |
| `YWCODER_CODE_USE_VERTEX` | 使用 Google Vertex AI |
| `YWCODER_CODE_SKIP_VERTEX_AUTH` | 跳过 Vertex Google 认证 |
| `VERTEX_REGION_*` | Vertex AI 按模型覆盖区域 |
| `YWCODER_CODE_USE_FOUNDRY` | 使用 Microsoft Foundry |
| `YWCODER_CODE_SKIP_FOUNDRY_AUTH` | 跳过 Foundry Azure 认证 |
...
| 环境变量 | 描述 |
|---------|------|
| `YWCODER_CODE_SHELL` | 覆盖 shell 检测 |
| `YWCODER_CODE_SHELL_PREFIX` | 命令前缀封装 |
| `YWCODER_BASH_MAINTAIN_PROJECT_WORKING_DIR` | 每条命令后返回原始目录 |
...
| 环境变量 | 描述 |
|---------|------|
| `YWCODER_CODE_ENABLE_TELEMETRY` | 启用 OpenTelemetry |
| `DISABLE_TELEMETRY` | 禁用 Statsig 遥测 |
| `DISABLE_ERROR_REPORTING` | 禁用 Sentry 错误报告 |
| `YWCODER_CODE_DISABLE_FEEDBACK_SURVEY` | 禁用会话质量调查 |
...
| 环境变量 | 描述 |
|---------|------|
| `YWCODER_CODE_ENABLE_PROMPT_SUGGESTION` | 启用/禁用 prompt 建议 |
| `YWCODER_CODE_ENABLE_TASKS` | 使用任务跟踪系统 |
| `YWCODER_CODE_DISABLE_AUTO_MEMORY` | 禁用自动记忆 |
| `YWCODER_CODE_DISABLE_BACKGROUND_TASKS` | 禁用后台任务 |
...
| `YWCODER_CODE_DISABLE_NONESSENTIAL_TRAFFIC` | 禁用所有非必要流量 |

### 3.9 文件 & 存储

| 环境变量 | 描述 |
|---------|------|
| `YWCODER_CONFIG_DIR` | 自定义配置/数据目录 |
| `YWCODER_CODE_TMPDIR` | 覆盖临时目录 |

### 3.10 其他

| 环境变量 | 描述 |
|---------|------|
| `YWCODER_CODE_HIDE_ACCOUNT_INFO` | 隐藏 email/org 信息 |
| `YWCODER_CODE_DISABLE_TERMINAL_TITLE` | 禁用终端标题更新 |
| `YWCODER_CODE_EXIT_AFTER_STOP_DELAY` | 自动退出延迟（ms） |
| `YWCODER_CODE_TASK_LIST_ID` | 跨会话共享任务列表 |
| `YWCODER_CODE_TEAM_NAME` | Agent team 名称 |
| `YWCODER_CODE_API_KEY_HELPER_TTL_MS` | 凭证刷新间隔（ms） |
| `YWCODER_CODE_AUTOCOMPACT_PCT_OVERRIDE` | 自动压缩触发百分比 (1-100) |
...
Alias 始终指向最新版本。要锁定特定版本，使用完整 model name（如 `ywcoder-opus-4-6`）。

### 4.2 模型选择优先级

1. **会话内** — `/model <alias|name>` 切换
2. **启动时** — `ywcoder --model <alias|name>`
...
| `YWCODER_CODE_SUBAGENT_MODEL` | 子代理 |
...
可通过 settings key `effortLevel`、环境变量 `YWCODER_CODE_EFFORT_LEVEL`、或 `/model` 滑块设置。
...
| Scope | 位置 | 用途 |
|-------|------|------|
| Local | `~/.ywcoder.json`（按项目路径存储） | 个人、当前项目 |
| Project | `.mcp.json`（项目根目录） | 团队共享（git 提交） |
| User | `~/.ywcoder.json`（全局区域） | 个人、跨项目 |
...
| 类型 | 位置 | 用途 | 共享范围 |
|------|------|------|---------|
| Managed policy | 系统目录 `YWCODER.md` | 企业级指令 | 所有用户 |
| User memory | `~/.ywcoder/YWCODER.md` | 个人全局偏好 | 仅自己 |
| User rules | `~/.ywcoder/rules/*.md` | 个人模块化规则 | 仅自己 |
| Project memory | `./YWCODER.md` 或 `./.ywcoder/YWCODER.md` | 项目指令 | 团队 (git) |
| Project rules | `./.ywcoder/rules/*.md` | 项目模块化规则 | 团队 (git) |
| Local memory | `./YWCODER.local.md` | 本地项目偏好 | 仅自己 (gitignored) |
| Auto memory | `~/.ywcoder/projects/<project>/memory/` | YwCoder 自动记录 | 仅自己 |

### 6.2 模块化规则 (.ywcoder/rules/)
...
存储在 `~/.ywcoder/projects/<project>/memory/` 下：
...
通过 `YWCODER_CODE_DISABLE_AUTO_MEMORY` 环境变量控制启用/禁用。

### 6.4 Import 语法

YWCODER.md 文件支持 `@path/to/import` 语法导入其他文件：

```markdown
See @README for project overview.
Individual preferences: @~/.ywcoder/my-project-instructions.md
```
...
| **Models** | `model`, `effortLevel`, `alwaysThinkingEnabled` | `ANTHROPIC_DEFAULT_*_MODEL`, `ANTHROPIC_MODEL`, `YWCODER_CODE_SUBAGENT_MODEL`, `MAX_THINKING_TOKENS` | ExtensionConfig: `customModels` |
...
| **MCP Servers** | `enableAllProjectMcpServers`, `enabledMcpjsonServers`, `disabledMcpjsonServers` | `MCP_TIMEOUT`, `MCP_TOOL_TIMEOUT`, `MAX_MCP_OUTPUT_TOKENS`, `ENABLE_TOOL_SEARCH` | **管道 C**: `~/.ywcoder.json`, `.mcp.json` |
| **Memory & Rules** | — | — | YWCODER.md 文件族, `.ywcoder/rules/*.md`, auto memory |
| **Environments** | `env` (排除已被其他 Tab 认领的 key) | 通用/未分类 env vars | — |
| **Sandbox** | `sandbox.*` | `BASH_DEFAULT_TIMEOUT_MS`, `BASH_MAX_TIMEOUT_MS`, `YWCODER_CODE_SHELL`, `BASH_MAX_OUTPUT_LENGTH` | — |
| **Network** | `skipWebFetchPreflight`, `remote` | `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`, `YWCODER_CODE_CLIENT_*`, 云供应商 vars | — |
...
- **显示**：`~/.ywcoder/settings.json`（或 `settings.{profile}.json`）的值
...
- **显示**：`.ywcoder/settings.local.json` 的值
...
Profile 切换只影响 User scope：
  ├── Profile "default"  → ~/.ywcoder/settings.json
  ├── Profile "aws"      → ~/.ywcoder/settings.aws.json
  └── Profile "vertex"   → ~/.ywcoder/settings.vertex.json
