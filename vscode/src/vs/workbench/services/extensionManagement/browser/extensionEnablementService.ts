...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
			...
			if (!builtinChatExtensionEnablementMigration) {
				...
				if (context) {
					if (context.value.state.completed) {
						...
						// [YwCoder] Skip auto-disabling builtin chat extension on first run.
						// In upstream VSCode this disables the extension if chat onboarding is incomplete,
						// but for our rebrand the chat extension IS the product, so we always keep it enabled.
						this.logService.debug('[YwCoder] Skipping builtin chat extension disable during migration');
					}
				}
			}
		}
	...
	private _isDisabledByUnification(identifier: IExtensionIdentifier): boolean {
		// [YwCoder] Skip disabling when completions and chat are the same extension.
		// In upstream VSCode this disables the completions ext when unification is on,
		// but for our rebrand there is only one extension serving both roles.
