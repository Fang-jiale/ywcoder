...
export class WebClientServer {
	...
	async handle(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery, pathname: string): Promise<void> {
		try {
			if (pathname.startsWith(STATIC_PATH) && pathname.charCodeAt(STATIC_PATH.length) === CharCode.Slash) {
				return this.handleStatic(req, res, pathname.substring(STATIC_PATH.length));
			}
			if (pathname === '/') {
	 ...
	 */
	async handleStatic(req: http.IncomingMessage, res: http.ServerResponse, resourcePath: string): Promise<void> {
	...
	private async _handleRoot(req: http.IncomingMessage, res: http.ServerResponse, parsedUrl: url.UrlWithParsedQuery): Promise<void> {
		...
		// [YwCoder] For local/offline/IP-based access, serve webviews from the same origin so
		// hashed subdomains like <uuid>.<ip> do not need special DNS resolution.
		const isLocalWebviewHost = /^(localhost|(?:\d{1,3}\.){3}\d{1,3}|\[[0-9a-fA-F:]+\]):\d+$/.test(webviewHost);
		...
		const webWorkerExtensionHostIframeScriptSHA = 'sha256-pERPYL/6WqtucDEE2l25EqG1v4DyGKl2kgbforDTYGU=';
		...
		const cspDirectives = [
			...
			`child-src 'self' ${webviewScheme}://*.${webviewHost};`,
			`frame-src 'self' ${webviewScheme}://*.${webviewHost} https://*.vscode-cdn.net data:;`,
