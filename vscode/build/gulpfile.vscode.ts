...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const deps = (await Promise.all([
			glob('**/*.node', { cwd, ignore: ['extensions/node_modules/@parcel/watcher/**', '**/ywcoder/dist/ywcoder/vendor/ripgrep/**', '**/node-pty/prebuilds/darwin-*/**', '**/node-pty/prebuilds/linux-*/**'] }),
			glob('**/rg.exe', { cwd }),
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(path.join(cwd, dep), {
				...
				'version-string': {
					...
					'LegalCopyright': 'Copyright (C) 2026 icbc. All rights reserved',
...
function prepareCopilotRipgrepShimTask(platform: string, arch: string, destinationFolderName: string) {
	...
	return async () => {
		...
		if (!fs.existsSync(builtInCopilotExtensionDir)) {
			console.log(`[prepareCopilotRipgrepShimTask] Copilot extension not found at ${builtInCopilotExtensionDir}, skipping.`);
			return;
		}
